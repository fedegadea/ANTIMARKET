...
export async function POST(request) {
  ...
  // Flujo simple: solo marca + email son obligatorios (el resto suma pero no bloquea).
  const brandName = str(body.brand_name, { max: 80 });
  const contactName = str(body.contact_name, { max: 80 }) || brandName;
  ...
  if (!brandName || !email) return err(400, 'missing fields');
