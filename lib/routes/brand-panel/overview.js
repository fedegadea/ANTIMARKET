...
// Auth: Supabase OTP; the user's email must match stores.contact_email (admins may
// pass ?store=<slug> to act on any store).
import { db } from '../../db.js';
import { json, err } from '../../http.js';
import { resolveStore } from './_store.js';

export async function GET(request) {
  const { store, errorResponse } = await resolveStore(request, 'id, name, slug, status, commission_shopping, commission_agent');
