-- Bloque 12 — Demora de envío por ZONA/PROVINCIA (reglas configurables por marca).
-- Pegar en Supabase → SQL Editor → Run. Idempotente. Requiere migracion_bloque11.sql.
-- shipping_rules: [{ "preset": "48h", "zones": ["CABA","Buenos Aires"] }, ...]
-- Una regla con "zones": [] es el default (resto del país).
alter table stores add column if not exists shipping_rules jsonb not null default '[]';
