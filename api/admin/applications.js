...
import { json, err, readJson, str, optStr, isEmail, clientIp } from '../../lib/http.js';
...
export async function GET(request) {
  ...
  return json({ applications: applications || [], active_stores: activeCount || 0, cap: 30 });
}
  ...
  if (status === 'invited') {
    ...
    await sendEmail(
      ...
      `<p>Hola ${escapeHtml(application.contact_name)},</p>
       <p>Tu marca <strong>${escapeHtml(application.brand_name)}</strong> fue invitada a Anti Market — la selección de 30 marcas argentinas.</p>
       ...
       <p>Tu catálogo, stock y precios se sincronizan solos. Cero costo fijo: 7% solo sobre ventas que te originamos, verificables orden por orden.</p>
